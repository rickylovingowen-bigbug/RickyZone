...
import { Download, Pencil, Plus, Trash2, Upload, X, ChevronDown, Search } from 'lucide-react';
...
function buildPayload(form: CharacterFormState): Omit<Character, 'id' | 'createdAt' | 'updatedAt'> {
  return {
    ...
    weapon: form.weapon.trim() || undefined, martialLevel: form.martialLevel || undefined,
...
const HEADER_MAP: Record<string, string> = {
  ...
  'Rank': 'martialLevel', 'rank': 'martialLevel', '样貌': 'appearance', '外貌': 'appearance',
  ...
  return (
    <div className="bg-bg-secondary border border-bg-tertiary rounded-xl overflow-hidden flex flex-col w-[160px] h-[240px]">
      <div className="px-3 pt-3 pb-2 flex items-center gap-1.5">
        <span className="px-1.5 py-0.5 rounded text-[10px] font-medium text-white" style={{ backgroundColor: rankColor }}>
          {character.martialLevel || '?'}
        </span>
        {character.sect && (
          <span className="px-1.5 py-0.5 rounded text-[10px] font-medium truncate max-w-[80px]" style={{ backgroundColor: sectColor, color: sectTextColor }}>
            {character.sect}
          </span>
        )}
      </div>
      <div className="px-3 py-1 text-center">
        <h3 className="text-base font-bold text-text-primary truncate">{character.name}</h3>
      </div>
      <div className="px-3 py-2 flex-1 space-y-1 text-xs">
        <div className="flex justify-between">
          <span className="text-text-tertiary">性别</span>
          <span className="text-text-primary">{character.gender === 'male' ? '男' : character.gender === 'female' ? '女' : '-'}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-text-tertiary">年龄</span>
          <span className="text-text-primary">{character.age ?? '-'}</span>
        </div>
        {character.sectPosition && (
          <div className="flex justify-between">
            <span className="text-text-tertiary">职位</span>
            <span className="text-text-primary truncate max-w-[70px]">{character.sectPosition}</span>
          </div>
        )}
        {character.weapon && (
          <div className="flex justify-between">
            <span className="text-text-tertiary">兵器</span>
            <span className="text-text-primary truncate max-w-[70px]">{character.weapon}</span>
          </div>
        )}
      </div>
      <div className="px-3 py-2 border-t border-bg-tertiary flex justify-center gap-3">
        <button onClick={() => onEdit(character)} className="p-1.5 rounded hover:bg-bg-tertiary" title="编辑">
          <Pencil className="w-4 h-4 text-text-secondary" />
        </button>
        <button onClick={() => onDelete(character.id)} className="p-1.5 rounded hover:bg-bg-tertiary" title="删除">
    ...
    try {
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        records.push({
          ...
          martialLevel, appearance: getValue('appearance'), personality: getValue('personality'),
          value: getValue('value'), conflict: getValue('conflict'), createdAt: now, updatedAt: now,
        });
      }
...
function CharacterForm({ form, onPatch, onSubmit, submitText, error, onCancel }: { form: CharacterFormState; onPatch: (patch: Partial<CharacterFormState>) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; submitText: string; error: string; onCancel: () => void; }) {
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <select value={form.martialLevel} onChange={e => onPatch({ martialLevel: e.target.value as Rank | '' })} className="rounded-lg border border-bg-tertiary bg-bg-primary px-3 py-2 text-text-primary"><option value="">武学 Rank</option>{RANK_ORDER.map(rank => <option key={rank} value={rank}>{rank}</option>)}</select>
