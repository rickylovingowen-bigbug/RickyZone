...
const LEGACY_RANK_MAP: Record<string, Rank> = {
  S: 'S',
  'A+': 'A+',
  A: 'A',
  'A-': 'A-',
...
function getSectColor(sect?: string): string {
  if (!sect) return '#6B7280';
  const palette = ['#2563EB', '#16A34A', '#DC2626', '#D97706', '#7C3AED', '#DB2777', '#0D9488'];
  const hash = [...sect].reduce((sum, char) => sum + char.charCodeAt(0), 0);
  return palette[hash % palette.length];
}

function getSectCardBackground(sect?: string): string {
  return `linear-gradient(165deg, rgba(31, 41, 55, 0.95), ${getSectColor(sect)}22)`;
}
...
function normalizeRank(rawRank?: string): Rank | undefined {
  if (!rawRank) return undefined;
  if (RANK_ORDER.includes(rawRank as Rank)) return rawRank as Rank;
  return LEGACY_RANK_MAP[rawRank];
}

function normalizeImportCharacters(payload: unknown): Partial<Character>[] {
  if (Array.isArray(payload)) return payload as Partial<Character>[];
  if (payload && typeof payload === 'object') {
    const record = payload as Record<string, unknown>;
    if (Array.isArray(record.characters)) {
      return record.characters as Partial<Character>[];
    }
    if (record.data && typeof record.data === 'object') {
      const nested = record.data as Record<string, unknown>;
      if (Array.isArray(nested.characters)) {
        return nested.characters as Partial<Character>[];
      }
    }
  }
  return [];
...
export default function CharacterManager() {
  ...
  const importCharactersFromFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (isJson) {
        ...
        const list = normalizeImportCharacters(payload);
        if (list.length === 0) {
          setError('导入失败：JSON 中未找到 characters 数据');
          return;
        }

        const existingNames = new Set((await db.characters.toArray()).map((item) => item.name));
        const now = new Date().toISOString();
        const records: Character[] = list
          .filter((item): item is Partial<Character> => typeof item?.name === 'string')
          .map((item) => {
            const uniqueName = makeUniqueName(item.name!.trim(), existingNames);
            return {
              id: generateId(),
              name: uniqueName,
              gender: item.gender,
              ...
              weapon: item.weapon,
              martialLevel: normalizeRank(item.martialLevel),
        ...
        if (records.length === 0) {
          setError('导入失败：JSON 没有可导入人物');
          return;
        }

        await db.characters.bulkAdd(records);
        setError('');
        alert(`导入完成，共 ${records.length} 条（JSON，兼容2.2/2.3/2.4）。`);
        return;
      }
      ...
      for (const row of rows.slice(1)) {
        ...
        records.push({
          ...
          martialLevel: normalizeRank(mapped.martialLevel),
      ...
      await db.characters.bulkAdd(records);
      setError('');
      alert(`导入完成，共 ${records.length} 条（CSV，兼容2.2/2.3/2.4）。`);
    } catch {
  ...
  const toggleAgeSort = () => {
    setFilter((prev) => ({ ...prev, ageSort: prev.ageSort === 'desc' ? 'asc' : 'desc' }));
  };

  const clearFilters = () => {
    setFilter({ ranks: [], sects: [], ageSort: 'desc' });
  };
    ...
    <div className="max-w-4xl mx-auto px-4 pb-8">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <label className="flex items-center gap-2 px-3 py-2 rounded-lg border border-bg-tertiary text-text-secondary cursor-pointer">
          ...
          导入 CSV / JSON
          <input type="file" accept=".csv,text/csv,.json,application/json" className="hidden" onChange={importCharactersFromFile} />
        </label>
      </div>
      ...
      <div className="space-y-3">
        {filteredCharacters.length === 0 ? (
          ...
          filteredCharacters.map((character) => (
            <div
              key={character.id}
              className="rounded-2xl p-4 flex items-center justify-between border shadow-md"
              style={{
                borderColor: getSectColor(character.sect),
                background: getSectCardBackground(character.sect),
              }}
            >
              <div>
